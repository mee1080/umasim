...
class CookScenarioEvents : CommonScenarioEvents() {
    ...
    override suspend fun afterAction(state: SimulationState, selector: ActionSelector): SimulationState {
        ...
        return when (state.turn) {
            ...
            24 -> if (cookStatus.cookPoint >= 1000) {
                base.levelUpBeforeEvent(selector)
                    .addStatus(Status(5, 5, 5, 5, 5, 40))
                    .updateCookStatus { updateDishRank(1, 1) }
                    .addRelation(5) { it.charaName == "秋川理事長" }
            } else {
                base.levelUpBeforeEvent(selector)
                    .addStatus(Status(3, 3, 3, 3, 3, 30))
            ...
            36 -> if (cookStatus.cookPoint >= 2500) {
                base.levelUpBeforeEvent(selector)
                    .addStatus(Status(10, 10, 10, 10, 10, 50))
            ...
            48 -> if (cookStatus.cookPoint >= 5000) {
                base.levelUpBeforeEvent(selector)
                    .addStatus(Status(15, 15, 15, 15, 15, 60))
            ...
            60 -> if (cookStatus.cookPoint >= 7000) {
                base.levelUpBeforeEvent(selector)
                    .addStatus(Status(20, 20, 20, 20, 20, 70))
    ...
    private suspend fun SimulationState.levelUpBeforeEvent(selector: ActionSelector): SimulationState {
        var newState = this
        var action: Action
        do {
