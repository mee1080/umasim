...
abstract class LegendCalculatorTest(
    ...
    protected val state = Simulator(
        ...
        LegendScenarioEvents().beforeSimulation(it)
    }
    ...
    protected fun Calculator.CalcInfo.updateLegendStatus(action: LegendStatus.() -> LegendStatus): Calculator.CalcInfo {
        return copy(scenarioStatus = legendStatus?.run { action() })
    }
    ...
    protected fun Calculator.CalcInfo.addBuff(name: String, enabled: Boolean = true) = updateLegendStatus {
        addBuff(getLegendBuff(name)!!, enabled)
    }

    protected fun Calculator.CalcInfo.setBuffEnabled(name: String, enabled: Boolean) = updateLegendStatus {
        copy(buffList = buffList.mapIf({ it.buff.name == name }) { it.copy(enabled = enabled) })
    }
    ...
    protected fun Calculator.CalcInfo.setBestFriendLevel(index: Int, level: Int) = setMemberState(index) {
        copy(scenarioState = (scenarioState as LegendMemberState).copy(bestFriendLevel = level))
    }

    protected fun Calculator.CalcInfo.setBestFriendGauge(index: Int, gauge: Int) = setMemberState(index) {
        copy(scenarioState = (scenarioState as LegendMemberState).copy(bestFriendGauge = gauge))
    }
}
