...
fun testExpected() {
    ...
    val member = Store.getSupportByName(
        ...
        MemberState(
            ...
            supportCard,
            StatusType.NONE,
            SupportState(relation = 100, hintIcon = false, passion = false, friendCount = 5),
    ...
    val expectedCalcInfo = ExpectedCalculator.ExpectedCalcInfo(
        chara, training, motivation, member, scenario, fanCount, currentStatus, totalRelation, liveStatus,
    )
...
//    val expected = ExpectedCalculator(expectedCalcInfo, listOf(StatusType.SPEED, StatusType.WISDOM)).calc()
    ...
    val calcInfo = Calculator.CalcInfo(
        ...
        member.distinctBy { it.card.type }.size, fanCount, currentStatus, totalRelation, liveStatus,
    )
